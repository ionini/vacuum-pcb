...
enum AutoPlacer {
    ...
    static func place(_ doc: CircuitDocument) -> [(componentId: UUID, position: Point)] {
        ...
        for placement in doc.physical.placements {
            ...
            let b = component.footprint(doc.manufacturing, snapshots: doc.librarySnapshots).boundingRect
        ...
        for net in doc.logic.nets {
            ...
            for pinRef in net.pins {
                guard positions[pinRef.componentId] != nil,
                      ...
                      let fp = component.footprint(doc.manufacturing, snapshots: doc.librarySnapshots).pin(pinRef.pinKey)
                else { continue }
