        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            group("Channels") {
                ...
                Text("Replaces the resistor zigzag with a rounded meander: parallel legs joined by tangent arcs, walls between neighbouring bores kept ≥ max(0.5, min wall) so a 0.2 mm nozzle can print them without smearing into the bore. Changes the channel length, so the simulated resistance shifts (L: ~34 → ~29 mm at defaults) — leave off for boards already printed/calibrated with the zigzag.")
                    .font(.caption2).foregroundStyle(.secondary)
                row("Resistor flare length", $draftMfg.resistorFlareLength)
                Text("Smooth L/XL only: each mouth is a cone opening from the resistor bore to the channel diameter at the pin, so the route joint prints the same whether it arrives along the resistor or from the side. This is also the straight lead before the meander — longer flares shrink the meander and lower the simulated resistance (L: ~29 mm at 1 mm, ~26 mm at 2 mm). 0 = no flare.")
                    .font(.caption2).foregroundStyle(.secondary)
